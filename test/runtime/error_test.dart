...
main() {
  group('RetryingRequestHandler', () {
    test('retries immediately on single 503 error', () {
      ...
      subject.handle(TEST_GET_REQUEST).first.then(expectAsync1((res) {
        expect(res, new isInstanceOf<RawEntity>());
      }));
    });
    test("doesn't retry on 404 error", () {
      ...
      subject.handle(TEST_GET_REQUEST).first.catchError(expectAsync1((e) {
