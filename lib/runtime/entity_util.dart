...
class ObservableImmutableListView implements ObservableList {
  ...
  Map<int, dynamic> asMap() => _delegate.asMap();
  void clear() => _throw();
  ...
  firstWhere(bool test(element), {orElse()}) => _delegate.firstWhere(test, orElse: orElse);
  ...
  int indexOf(element, [int startIndex = 0]) => _delegate.indexOf(element, startIndex);
  void insert(_a, _b) => _throw();
  void insertAll(_a, _b) => _throw();
  String join([String separator = '']) => _delegate.join(separator);
  int lastIndexOf(element, [int startIndex]) => _delegate.lastIndexOf(element, startIndex);
  lastWhere(bool test(element), {orElse()}) => _delegate.lastWhere(test, orElse: orElse);
